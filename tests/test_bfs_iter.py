...
def test_bfs_iter(random_list_tree):
    expected_values = []
    ...
    for a,b in zip(BFSIter(random_list_tree, use_seen=False), expected_values):
        assert a == b

def test_bfs_iter_random(random_tree):
    expected_values = []
    ...
    while len(queue) > 0:
        ...
        for child in current.children:
            queue.append((depth + 1, child))
        expected_values.append((depth, current))
    for a,b in zip(BFSIter(random_tree,use_seen=False,child_attribute="children"), expected_values):
        assert a == b
